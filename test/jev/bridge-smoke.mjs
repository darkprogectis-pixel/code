...
async function scenario(name, port, extra) {
  const child = spawn(process.execPath, [CLI, '--serve', '--port', String(port), '--cycles', '2', ...extra], { stdio: ['ignore', 'ignore', 'pipe'] });
  ...
  if (state) {
    ...
    set('CONTROL_CENTER_UI', ui.status === 200 && /JEV Control Center/.test(ui.body), name + ' ui');
    ...
    set('ROBOT_LOCKED_OFF', state.robot.state === 'OFF' && state.robot.can_enable === false && Object.values(state.robot.safety).every((v) => v === false), name + ' robo');
...
await scenario('LIVE', 3592, ['--interval-ms', '5000']);
await scenario('REPLAY', 3593, ['--replay', path.join(REPO_ROOT, 'fixtures', 'jev', 'C_valid_multi_source.json'), '--interval-ms', '1000']);
console.log('');
const keys = ['PROCESS_START', 'BRIDGE_READ_ONLY', 'CONTROL_CENTER_UI', 'PANEL_STATE', 'ROBOT_LOCKED_OFF', 'NO_ORDER', 'SAME_ENGINE_OUTPUT'];
